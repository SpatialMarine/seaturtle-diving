# Plot individual maps with binary MPM classification

# 1) Plot ids in West-Mediterranean
# 2) Plot ids in Atlantic and all Mediterranean Sea extent
...
for (i in seq_along(ids)) {
  ...
  # load estate space models position with MPM values 
  locs <- read.csv(paste0(output_dir,"/tracking/locdata/L2_loc/",id,"_L2_loc.csv"))
  
  g_threshold <- median(locs$g)
  
  # classify MPM values based in 0.78 threshold
  locs$mpm <- ifelse(locs$g >= g_threshold, "≥ g_threshold", "< g_threshold")
  locs$mpm <- as.factor(locs$mpm)
  
  # and obtain first and last track position from STMs
  start <- head(locs, 1)
  end  <- tail(locs, 1)
  ...
  p <- ggplot() +
    ...
    # states values
    geom_point(aes(x=longitude, y=latitude, col=as.factor(mpm)),
               data=locs,
               size = 2.55,
               color = "grey25",
               alpha = 0.1) + 
    geom_point(aes(x=longitude, y=latitude, col=as.factor(mpm)),
               ...
               color = "grey10",
               alpha = 0.8) + 
    geom_point(aes(x=longitude, y=latitude, col=as.factor(mpm)), #in aes, shape=as.factor(class)
               data=locs,
               size = 1.6) + 
    scale_color_manual(name = "MPM", labels = c("≥ MPM threshold", "< MPM threshold"), , values = c("≥ g_threshold" = "#ffb000", "< g_threshold" = "#5D336D")) +
    ...
    geom_text(aes(x = -2.3, y = 42.7, 
                   label = paste("MPM threshold:", sprintf("%.2f", g_threshold))), 
               size = 4, color = "grey20") +
  ...
  # export map
  p_png <- paste0(output_dir,"/fig/ai/sup_map2-3/ptt/",id,"_sup_map2-3.png")
  # p_svg <- paste0(output_dir,"/fig/ai/sup_map2-3/ptt/",id,"_sup_map2-3.svg")
...
p <- ggplot() +
  ...
  geom_label(aes(x = 5, y = 15, label = paste("ptt:", id)), 
